$(function () {
	var boxModel = {
		$box: $('.box'),
		$boxInner: $(
			'<div class="box-inner box-property" data-property="content"></div>'
		),
		$boxMargin: $(
			'<div class="box-margin box-property" data-property="margin"></div>'
		),
		$boxPadding: $(
			'<div class="box-padding box-property" data-property="padding"></div>'
		),
		$boxBorder: $(
			'<div class="box-border box-property" data-property="border"></div>'
		),
		...
		prependElements: function () {
			...
			$('.box-property').append(
				'<span class="box-property-vertical"></span><span class="box-property-horizontal"></span>'
			)
		...
		getBoxProperties: function () {
			...
			boxModel.boxSizing = $('#borderBox').is(':checked')
				? 'border-box'
				: 'content-box'
			...
			boxModel.boxPaddingBottom = parseInt(
				$('#boxPaddingBottom').val(),
				10
			)
		...
		linkProperties: function (properties) {
			...
			for (i = 0; i < properties.length; i++) {
				...
				if (linkTB) {
					...
					boxModel['box' + properties[i] + 'Top'] =
						boxModel['box' + properties[i] + 'Top']
					boxModel['box' + properties[i] + 'Bottom'] =
						boxModel['box' + properties[i] + 'Top']
					$('#box' + properties[i] + 'Bottom').val(
						boxModel['box' + properties[i] + 'Top']
					)
				}
				if (linkRL) {
					boxModel['linked' + properties[i] + 'RL'] = true
					boxModel['box' + properties[i] + 'Left'] =
						boxModel['box' + properties[i] + 'Right']
					boxModel['box' + properties[i] + 'Left'] =
						boxModel['box' + properties[i] + 'Left']
					$('#box' + properties[i] + 'Left').val(
						boxModel['box' + properties[i] + 'Left']
					)
				}
				if (linkAll) {
		...
		addDimensionsToModel: function () {
			...
			for (i = 0; i < properties.length; i++) {
				boxModel['$box' + properties[i]]
					.find('.box-property-vertical')
					.attr({
						'data-top': boxModel['box' + properties[i] + 'Top'],
						'data-bottom':
							boxModel['box' + properties[i] + 'Bottom'],
					})
					.next('.box-property-horizontal')
					.attr({
		...
		generateShorthand: function (property) {
			var topAndBottomMatch =
				boxModel['box' + property + 'Top'] ===
				boxModel['box' + property + 'Bottom']
			var topAndBottomNoMatch =
				boxModel['box' + property + 'Top'] !==
				boxModel['box' + property + 'Bottom']
			var leftAndRightMatch =
				boxModel['box' + property + 'Left'] ===
				boxModel['box' + property + 'Right']
			var allMatch =
				boxModel['box' + property + 'Top'] +
					boxModel['box' + property + 'Bottom'] ===
					boxModel['box' + property + 'Left'] +
		...
		generateCode: function () {
			...
			boxCode +=
				'    width: <span contenteditable type="number" pattern="[0-9]{1,3}">' +
				boxModel.boxWidth +
				'</span>px;\n'
			...
			boxCode +=
				'    border-width: ' + boxModel.generateShorthand('Border')
		...
		positionBoxProperties: function () {
			...
			var boxMarginLeftRight =
				boxModel.boxMarginLeft + boxModel.boxMarginRight
			var boxMarginTopBottom =
				boxModel.boxMarginTop + boxModel.boxMarginBottom

			// Padding
			var boxPaddingTop = boxModel.boxPaddingTop
			var boxPaddingLeft = boxModel.boxPaddingLeft
			var boxPaddingLeftRight =
				boxModel.boxPaddingLeft + boxModel.boxPaddingRight
			var boxPaddingTopBottom =
				boxModel.boxPaddingTop + boxModel.boxPaddingBottom

			// Border
			var boxBorderTop = boxModel.boxBorderTop
			var boxBorderLeft = boxModel.boxBorderLeft
			var boxBorderLeftRight =
				boxModel.boxBorderLeft + boxModel.boxBorderRight
			var boxBorderTopBottom =
				boxModel.boxBorderTop + boxModel.boxBorderBottom
			...
			// Border Box
			var boxBorderWidth =
				boxWidth + boxPaddingLeftRight + boxBorderLeftRight
			var boxBorderHeight =
				boxHeight + boxPaddingTopBottom + boxBorderTopBottom
			...
			if (boxMarginLeft >= 0) {
				boxMarginLeft =
					boxMarginLeft * -1 - boxPaddingLeft - boxBorderLeft
			}
			if (boxBorderTop >= 0) {
